...
import CharactersLeftDisplay, { GetCharacterCountRemaining } from './CharactersLeftDisplay';
...
it("renders without crashing", () => {
    act(() => {
        render(<SubmissionTextBox snappables={[{ id: 'test-id', display: 'test-display' }]} user={'test-user'} />, container);
    });
...
describe('CharactersLeftDisplay', () => {
    test("characters remaining string renders", () => {
        ...
        const elem2: MentionElements = { id: "anna@softwire.com", display: "Anna" }
        const msg2: String = "Hi there @[Anna](anna@softwire.com) and @[Jim](idlength10), you have cool voices!"
        const component = renderer.create(<CharactersLeftDisplay snappedUsers={[elem1, elem2]} message={msg2} />);
    ...
    test("characters remaining string renders", () => {
        ...
        const elem2: MentionElements = { id: "anna@softwire.com", display: "Anna" }
        const msg1: String = ""
        const component = renderer.create(<CharactersLeftDisplay snappedUsers={[]} message={msg1} />);
        let tree = component.toJSON();
        expect(tree).toMatchSnapshot();
    });
})
...
describe("<SubmissionTextBox />", () => {
    test("test for successful submission of a legitimate snap", () => {
        ...
        const elem1: MentionElements = { id: "RandomUNiqueID1", display: "Jimathy" }
        ...
        const validSnap1: Snap = { body: message1, to: [], from: "Timathy", timestamp: new Date }
        const validSnap2: Snap = { body: message2, to: ["RandomUNiqueID1"], from: "Timathy", timestamp: new Date }
    ...
    test("test for character count changes on type and appearance of characters left", () => {
        ...
        */
        ...
        const message1: String = "Hello mate, how are you?"
        const message2: String = "Hello @(Jimathy)[RandomUNiqueID1], you are cool"
        expect(GetCharacterCountRemaining(message1, emptyMentions)).toBe(280 - message1.length)
        expect(GetCharacterCountRemaining(message2, someMentions)).toBe(280 - message2.length + "@()[]".length + elem1.id.length)
    })
    ...
    test("test for character limit changes when tagged users changes", () => {
        ...
        */
        const emptyMentions: MentionElements[] = []
        const elem1: MentionElements = { id: "RandomUNiqueID1", display: "Jimathy" }
        const elem2: MentionElements = { id: "RandomUNiqueID2", display: "Jonathy" }
        const elem3: MentionElements = { id: "RandomUNiqueID2", display: "Jenathy" }
        const someMentions: MentionElements[] = [elem1, elem2, elem3]
        expect(GetExtraLength(emptyMentions)).toBe(280)
        expect(GetExtraLength(someMentions)).toBe(280 + 3 * (elem1.id.length + 5))
    })
})
